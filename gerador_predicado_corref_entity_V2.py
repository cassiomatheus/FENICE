...
"""
gerador_predicado_corref_v2.py
Gera resumos corrompidos com erros de PREDICADO (Verbos) e CORREFERÊNCIA (Pronomes).
Utiliza Seleção Prioritária por Dicionário para garantir a ocorrência de Antônimos de domínio.
"""
...
def estrategizar_predicado(verbo_alvo, sent, antonimos):
    ...
    # -----------------------------
    # 2. NEGAÇÃO CORRETA (Sua Lógica Avançada)
    # -----------------------------
    ...
    # -----------------------------
    # 3. SHIFT DE TEMPO (Corrigido para padrão inglês)
    # -----------------------------
...
def inject_realistic_predicate_errors(
    ...
    # Separação prioritária para garantir a injeção dos antônimos
    candidatos_vip = []
    candidatos_normais = []

    for i, sent in enumerate(sents):
        verbos = [t for t in sent if t.pos_ == "VERB" and t.is_alpha]
        if verbos:
            # Usa a variável global ANTONIMOS_DOMINIO
            if any(v.lemma_.lower() in ANTONIMOS_DOMINIO for v in verbos):
                candidatos_vip.append((i, sent, verbos))
            else:
                candidatos_normais.append((i, sent, verbos))
    ...
    # Preenche primeiro com as sentenças que possuem antônimos
    if candidatos_vip:
        qtd_vip = min(len(candidatos_vip), num_erros)
        escolhidos.extend(random.sample(candidatos_vip, qtd_vip))
        
    # Completa as vagas restantes com sentenças comuns
    ...
    for i, sent in enumerate(sents):
        ...
        if match:
            ...
            # Força a escolha do verbo que tem antônimo, se ele existir na frase
...
def inject_realistic_coreference_errors(resumo_orig: str, fonte_orig: str, num_erros: int = 2):
    ...
    for i, sent in enumerate(sentencas):
        for t in sent:
            # O spaCy classifica o "expletive it" como 'expl', logo a checagem abaixo já o ignora naturalmente
            if (
                t.pos_ == "PRON"
    ...
    # Ordena os alvos escolhidos do FIM para o INÍCIO do texto.
    # CORREÇÃO CRÍTICA: Mudar o texto de trás para frente preserva os índices de caracteres dos tokens anteriores!
    ...
    for sent_idx, sent_ref, pron in escolhidos:
        # Antecedente original (se houver coref resolver)
        antecedente_real = None

        candidatos_validos = [c for c in candidatos if c["lemma"] not in usados]
        ...
        # CORREÇÃO BIÔNICA: Substituição exata baseada na posição do caractere no texto global
        start_char = pron.idx
        ...
        erros.append({
            ...
            "tipo": "coreference_long_range_loss"
        })

    # Como processamos de trás para frente, invertemos a lista de erros para manter a ordem cronológica no CSV
    erros.reverse()
...
def inject_realistic_entity_errors(resumo_orig: str, fonte_orig: str, num_erros: int = 2):
    doc_resumo = nlp(resumo_orig)
    doc_fonte = nlp(fonte_orig)
    ...
    for i, sent in enumerate(sentencas):
        ...
        if i in escolhidas:  # <-- Linha corrigida aqui!
            ents = list(sent.ents)
            ...
            # Logs padronizados para o metavaliacao2.py
            logs.append({
                "sentenca_index": i,
                "tipo_erro": "entity",
                "original": alvo.text,
...
if __name__ == "__main__":
    ...
    print("\nExemplo Predicado Atualizado:")
    print(df[['id', 'qtd_erros_predicado', 'tokens_predicado']].head())
    
    df.to_csv('govreport_erros_pred_corref_entity_realistas.csv', index=False)