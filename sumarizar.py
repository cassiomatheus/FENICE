...
class LongDocumentSummarizer:
    ...
    # CORREÇÃO: length_penalty alterado para 1.0 (neutro) para evitar "inchaço" artificial do texto
    def summarize(self, text: str, num_beams: int = 4, length_penalty: float = 1.0,
        ...
        if "led" in self.model_name.lower() or "primera" in self.model_name.lower(): #Para garantir a mesma otimização de atenção.
            global_attention_mask = self._create_global_attention_mask(input_ids)

        # 4. Determinar tamanho alvo
        # CORREÇÃO: Remoção do limite dinâmico de 8% que causava o colapso e repetição infinita
        alvo_min = 50 
        alvo_max = 512
...
def process_dataset(model_name: str, dataset_name: str, output_file: str,
    ...
    # Coleta metadados da execução (Atualizado com os novos parâmetros estáticos)
    metadados = {
        "modelo": model_name,
...
if __name__ == "__main__":
    ...
    # CORREÇÃO: O modelo default agora é uma versão fine-tuned no dataset GovReport para evitar geração zero-shot errática.
    parser.add_argument("--modelo", type=str, default="allenai/led-large-16384-govreport",
    ...
    parser.add_argument("--output", type=str, default="Resultados_Sumarizacao/resultados.jsonl",
